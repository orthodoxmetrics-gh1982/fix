...
router.post('/generate', async (req, res) => {
    try {
        ...
        switch (serviceType.toLowerCase()) {
            case 'baptism':
                const [baptismRows] = await getAppPool().query(
            ...
            case 'marriage':
                const [marriageRows] = await getAppPool().query(
            ...
            case 'funeral':
                const [funeralRows] = await getAppPool().query(
...
router.post('/', requireAuth, requireChurchBilling(), async (req, res) => {
    try {
        ...
        line_total, tax_rate, tax_amount, sort_order
            ...
            await getAppPool().query(itemsQuery, [itemsValues]);
        }

        // Get the created invoice
        const [newInvoice] = await getAppPool().query(
            `SELECT 
        i.*,
        c.name as church_name
       FROM invoices i 
...
router.put('/:id', async (req, res) => {
    try {
       ...
       WHERE id = ?`,
        ...
        // Get updated invoice
        const [updatedInvoice] = await getAppPool().query(
            `SELECT 
        i.*,
        c.name as church_name
       FROM invoices i 
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const [result] = await getAppPool().query('DELETE FROM invoices WHERE id = ?', [id]);
