-- OrthodoxMetrics AI (OMAI) Database Schema
-- Dedicated database for Big Book knowledge management system

-- Create the OMAI database
CREATE DATABASE IF NOT EXISTS omai_db
...
('database_name', 'omai_db', 'string', 'OMAI database name', TRUE),
('main_database_name', 'orthodoxmetrics_db', 'string', 'Main OrthodoxMetrics database name', TRUE);

-- Insert default watchers
INSERT IGNORE INTO bigbook_watchers (watch_path, file_patterns, exclude_patterns) VALUES
('/var/www/orthodox-church-mgmt/orthodoxmetrics/prod', '["**/*.md", "**/*.sql", "**/*.js", "**/*.ts", "**/*.sh", "**/*.ps1"]', '["**/node_modules/**", "**/.git/**", "**/logs/**", "**/temp/**", "**/bigbook/**"]'),
...
-- Create OMAI database user (optional - for production security)
-- CREATE USER IF NOT EXISTS 'omai_user'@'localhost' IDENTIFIED BY 'your_secure_password';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON omai_db.* TO 'omai_user'@'localhost';
-- FLUSH PRIVILEGES;
...
INSERT IGNORE INTO bigbook_config (config_key, config_value, config_type, description, is_system) VALUES
('system_name', 'OrthodoxMetrics AI (OMAI)', 'string', 'System name', TRUE),
('system_version', '1.0.0', 'string', 'System version', TRUE),
('database_version', '1.0.0', 'string', 'Database schema version', TRUE),
('created_date', NOW(), 'string', 'Database creation date', TRUE),
('last_updated', NOW(), 'string', 'Last system update', TRUE);

-- =====================================================
-- MARKDOWN INGESTION SYSTEM TABLES (Task 139)
-- =====================================================
...
-- Update system configuration for markdown ingestion
INSERT IGNORE INTO bigbook_config (config_key, config_value, config_type, description, is_system) VALUES
('md_ingestion_enabled', 'true', 'boolean', 'Enable markdown file ingestion', TRUE),
('md_max_file_size', '10485760', 'number', 'Maximum markdown file size in bytes (10MB)', TRUE),
('md_storage_path', '../../misc/omai/bigbook/docs', 'string', 'Storage path for ingested markdown files', TRUE),
('md_auto_parse', 'true', 'boolean', 'Automatically parse ingested markdown files', TRUE),
('md_auto_index', 'true', 'boolean', 'Automatically index parsed markdown for search', TRUE),
('grep_enabled', 'true', 'boolean', 'Enable AI-Grep search functionality', TRUE),
('search_similarity_threshold', '0.7', 'number', 'Minimum similarity score for search results', TRUE),
('agent_detection_enabled', 'true', 'boolean', 'Enable automatic agent reference detection', TRUE); 