...
const getLocalizedService = async (serviceCode, language = 'en') => {
  try {
    ...
    if (language !== 'en') {
      const [fallbackRows] = await getAppPool().query(
...
router.get('/:id', async (req, res) => {
  try {
    ...
    // Get invoice details
    const [invoiceRows] = await getAppPool().query(`
      SELECT 
        i.*,
        c.name as church_name,
        c.email as church_email,
    ...
    // Get invoice items
    const [itemRows] = await getAppPool().query(`
      SELECT *
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const [createdInvoice] = await getAppPool().query(`
      SELECT 
        i.*,
        c.name as church_name,
        c.email as church_email
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const [existingInvoice] = await getAppPool().query(
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await getAppPool().query('DELETE FROM invoices WHERE id = ?', [id]);
...
router.post('/:id/items', async (req, res) => {
  try {
    ...
    // Get next sort order
    const [maxOrderRows] = await getAppPool().query(
...
router.get('/service-catalog', async (req, res) => {
  try {
    ...
    const [services] = await getAppPool().query(`
      SELECT 
        service_code,
        category,
