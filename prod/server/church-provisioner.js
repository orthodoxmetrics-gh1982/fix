...
const { info, warn, error, success } = require('./utils/dbLogger');
...
class ChurchProvisioner {
  ...
  async createChurchDatabase(churchData) {
    ...
    try {
      await info('ChurchProvisioner', 'Starting church provisioning process', churchData, null, 'church-provisioner');
      
      // 1. Generate database name
      const dbName = this.generateDbName(churchData.name);
      await info('ChurchProvisioner', `Generated database name: ${dbName}`, { 
        churchName: churchData.name, 
        dbName 
      }, null, 'church-provisioner');
      
      // 2. Create database
      await connection.execute(`CREATE DATABASE IF NOT EXISTS \`${dbName}\``);
      await success('ChurchProvisioner', `Database "${dbName}" created successfully`, { 
        dbName 
      }, null, 'church-provisioner');
