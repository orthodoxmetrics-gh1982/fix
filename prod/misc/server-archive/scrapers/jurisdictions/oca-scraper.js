...
class OCAScraper extends BaseScraper {
    ...
    async scrapeParishDirectory() {
        ...
        try {
            ...
            const html = await this.fetchWithPuppeteer('https://www.oca.org/parishes/search', {
                waitForSelector: 'article.category.w-thumbnail.clearfix',
    ...
    'article.category.w-thumbnail.clearfix'
];
    ...
    const cityStateText = $el.find('p em').first().text().trim();
    let city = null, state = null;
    if (cityStateText.includes(',')) {
        [city, state] = cityStateText.split(',').map(s => s.trim());
    } else {
        city = cityStateText;
    }

    const dioceseDeaneryRaw = $el.find('p').eq(1).text().trim();
    const dioceseDeaneryText = dioceseDeaneryRaw.split(/\n/).map(t => t.trim()).filter(Boolean);
    const diocese = dioceseDeaneryText[0] || null;
    const deanery = dioceseDeaneryText[1] || null;

    const website = $el.find('a[href^="http"]').attr('href') || null;

    return this.standardizeChurchData({
        name: name,
        city: city,
        state: state,
        diocese: diocese,
