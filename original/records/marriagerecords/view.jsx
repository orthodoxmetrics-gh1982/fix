...
import useMarriageRecords from "@/hooks/useMarriageRecords";
...
const ReadOnlyView = () => {
  ...
  // Use the marriage records hook
  const { records, loading: isLoading, error } = useMarriageRecords();
  ...
  // Read-only column definitions based on marriage records schema
  const baseCols = [
    {
      headerName: 'Groom',
      field: 'fname_groom',
      flex: 1,
      editable: false,
      valueFormatter: p => `${p.data.fname_groom || ''} ${p.data.lname_groom || ''}`.trim(),
    },
    {
      headerName: 'Bride',
      field: 'fname_bride',
      flex: 1,
      editable: false,
      valueFormatter: p => `${p.data.fname_bride || ''} ${p.data.lname_bride || ''}`.trim(),
    },
    {
  ...
  function getResponsiveColumnDefs() {
    ...
    if (w < 768) {
      return baseCols.filter(
        c => ['fname_groom', 'fname_bride', 'mdate'].includes(c.field)
      );
    }
    if (w < 1024) {
      return baseCols.filter(
        c => ['fname_groom', 'fname_bride', 'mdate', 'clergy'].includes(c.field)
      );
    }
    return baseCols;
  }
    ...
    return (
      <div className="d-flex justify-content-center align-items-center" style={{ height: '400px' }}>
        <div className="text-center">
          ...
          <div className="mt-2">Loading marriage records...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="read-only-view" style={{
      ...
      <Alert variant="info" className="mb-3" style={{
        ...
        <strong>🔒 Read-Only Mode:</strong> You are viewing marriage records in read-only mode.
        ...
        <Card.Body className="p-2">
          <Form onSubmit={handleSearch}>
            <div className="d-flex gap-2">
              <Form.Control
                type="text"
                placeholder="Search by groom, bride, or clergy..."
                value={searchTerm}
