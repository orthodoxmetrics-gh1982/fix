...
router.get('/:slug', async (req, res) => {
  try {
    ...
    const [pages] = await promisePool.query(query, [slug]);
...
router.put('/:slug', async (req, res) => {
  try {
    ...
    const [existingPages] = await promisePool.query(existingPageQuery, [slug]);
    ...
    if (existingPages.length > 0) {
      ...
      await promisePool.query(updateQuery, [title, content || '', meta_description || '', slug]);
      ...
      const fetchQuery = `
        ...
        WHERE slug = ?
      `;
      const [updatedPages] = await promisePool.query(fetchQuery, [slug]);
      ...
      const [result] = await promisePool.query(insertQuery, [slug, title, content || '', meta_description || '']);
      ...
      const fetchQuery = `
        ...
        WHERE id = ?
      `;
      const [createdPages] = await promisePool.query(fetchQuery, [result.insertId]);
...
router.get('/', async (req, res) => {
  try {
    ...
    const [pages] = await promisePool.query(query, [limit, offset]);
...
router.delete('/:slug', async (req, res) => {
  try {
    ...
    const [result] = await promisePool.query(deleteQuery, [slug]);
...
router.post('/:slug/duplicate', async (req, res) => {
  try {
    ...
    const sourceQuery = `
      ...
      WHERE slug = ?
    `;
    const [sourcePages] = await promisePool.query(sourceQuery, [slug]);
