...
router.get('/:slug', async (req, res) => {
  try {
    ...
    const pages = await db.query(query, [slug]);
...
router.put('/:slug', async (req, res) => {
  try {
    ...
    const existingPages = await db.query(existingPageQuery, [slug]);
    ...
    if (existingPages.length > 0) {
      ...
      await db.query(updateQuery, [title, content || '', meta_description || '', slug]);
      ...
      const fetchQuery = `
        ...
        WHERE slug = ?
      `;
      const updatedPages = await db.query(fetchQuery, [slug]);
      ...
      result = await db.query(insertQuery, [slug, title, content || '', meta_description || '']);
      ...
      const fetchQuery = `
        ...
        WHERE id = ?
      `;
      const createdPages = await db.query(fetchQuery, [result.insertId]);
...
router.get('/', async (req, res) => {
  try {
    ...
    const pages = await db.query(query, [limit, offset]);
...
router.delete('/:slug', async (req, res) => {
  try {
    ...
    const result = await db.query(deleteQuery, [slug]);
...
router.post('/:slug/duplicate', async (req, res) => {
  try {
    ...
    const sourceQuery = `
      ...
      WHERE slug = ?
    `;
    const sourcePages = await db.query(sourceQuery, [slug]);
