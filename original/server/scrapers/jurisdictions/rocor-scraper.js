// 📁 server/scrapers/jurisdictions/rocor-scraper.js
// Russian Orthodox Church Outside Russia (ROCOR) scraper

const BaseScraper = require('./base-scraper');

class ROCORScraper extends BaseScraper {
    constructor(options = {}) {
        super({
            jurisdiction: 'Russian Orthodox Church Outside Russia (ROCOR)',
            baseUrl: 'https://www.synod.com',
            ...options
        });
    }
    ...
    async scrapeChurches() {
        ...
        try {
            this.logProgress('Starting ROCOR parish scraping');
            
            const directoryChurches = await this.scrapeParishDirectory();
            churches.push(...directoryChurches);
            
            this.logProgress('ROCOR scraping completed', { 
            ...
            this.logError('Failed to scrape ROCOR parishes', error);
            return churches;
        }
    }
    ...
    async scrapeParishDirectory() {
        ...
        try {
            const directoryUrls = [
                ...
                'https://www.synod.com/directory',
                'https://www.synod.com/parish-directory'
            ];
            ...
            for (const url of directoryUrls) {
                try {
                    ...
                    $('.parish, .church, .parish-listing, .directory-item').each((index, element) => {
                        const church = this.extractROCORChurch($, element);
            ...
            this.logError('Error scraping ROCOR directory', error);
        }
        
        return churches;
    }

    extractROCORChurch($, element) {
        ...
        const rawData = {
            ...
            website: $element.find('a[href*="http"]:not([href*="synod.com"])').first().attr('href'),
            ...
            source_url: 'https://www.synod.com'
        };
