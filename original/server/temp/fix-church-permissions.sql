...
-- Grant full permissions to orthodoxapp user for all church databases
GRANT ALL PRIVILEGES ON `saints_peter_and_paul_orthodox_church_db`.* TO 'orthodoxapp'@'localhost';

-- Grant permissions for future church databases (wildcard pattern)
GRANT ALL PRIVILEGES ON `*_db`.* TO 'orthodoxapp'@'localhost';

-- Grant specific permissions needed for OCR functionality
GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, ALTER, INDEX ON `saints_peter_and_paul_orthodox_church_db`.* TO 'orthodoxapp'@'localhost';

-- Flush privileges to apply changes
FLUSH PRIVILEGES;

-- Show current grants for orthodoxapp user
SHOW GRANTS FOR 'orthodoxapp'@'localhost';
