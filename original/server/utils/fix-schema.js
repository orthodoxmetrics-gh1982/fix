...
async function fixUserSchema() {
  try {
    ...
    if (tables.length === 0) {
      ...
      await promisePool.query("UPDATE users SET role = 'admin' WHERE role IS NULL OR role = ''");
      console.log('✅ Updated existing users with admin role');
    }

    // Ensure admin user exists
    const [adminUsers] = await promisePool.query("SELECT id FROM users WHERE email = 'admin' OR username = 'admin'");
    ...
    if (adminUsers.length === 0) {
      ...
      await promisePool.query(`
        INSERT INTO users (email, username, password_hash, role, landing_page) 
