...
const InvoiceViewByNumber: React.FC = () => {
    const { invoice_number } = useParams<{ invoice_number: string }>();
    const navigate = useNavigate();
    const { getInvoiceByNumber, markInvoiceAsPaid, generateInvoicePDF, sendInvoiceEmail } = useInvoiceContext();
    ...
    useEffect(() => {
        console.log('Component mounted, Invoice Number from params:', invoice_number);
        if (invoice_number) {
            fetchInvoice();
        }
    }, [invoice_number]);
    ...
    const fetchInvoice = async () => {
        try {
            setLoading(true);
            setError(null);
            console.log('Fetching invoice with number:', invoice_number);
            ...
            <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                ...
                <Stack direction="row" spacing={1}>
                    <Tooltip title="Edit Invoice">
                        <IconButton
                            color="primary"
                            onClick={() => navigate(`/apps/invoice/edit/${invoice.invoice_number}`)}
                        >
                            <IconEdit />
                        </IconButton>
                    </Tooltip>
                                ...
                                {invoice.church && (
                                    <>
                                        <Typography variant="body2" color="text.secondary" mt={1}>
                                            Customer ID: <Typography component="span" fontWeight="medium" color="primary.main">{invoice.church.customer_id || 'N/A'}</Typography>
                                        </Typography>
                                        <Typography variant="body2" color="text.secondary" mt={1}>
...
export default InvoiceViewByNumber;
