...
const Cart = () => {
  ...
  // Handle case where cartItems might be undefined
  const safeCartItems = cartItems || [];
  const bcount = safeCartItems.length > 0 ? safeCartItems.length : '0';

  const total = sum(safeCartItems.map((product: any) => product.price * product.qty));
